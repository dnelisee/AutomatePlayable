const fs = require('fs');
const file3 = 'c:/Users/orean/OneDrive/Documents/hackaton/AutomatePlayable/Playables ads/playable3.html';
const file4 = 'c:/Users/orean/OneDrive/Documents/hackaton/AutomatePlayable/Playables ads/playable4.html';
const content = fs.readFileSync(file3, 'utf8');
...
const CAM_MODE = {OVERVIEW:0, TRACKING:1, ZOOM_UNIT: 2};
...
function nextTurn() {
  ...
  blue.facadeAlpha = blue.isDestroyed ? 0 : 1;
  red.facadeAlpha = red.isDestroyed ? 0 : 1;
  state = S.OVERVIEW;
  phaseTimer = 0;
  isDragging = false;
...
function updatePhase(dt) {
  ...
  switch(state) {
    case S.OVERVIEW:
      ...
      break;
    case S.FOCUS:
      if (phaseTimer > 0.8) { state = S.REVEAL; phaseTimer = 0; }
      break;
    case S.REVEAL:
      if(!activeCastle.isDestroyed) {
          activeCastle.facadeAlpha -= dt * 3.0;
          if (activeCastle.facadeAlpha <= 0) {
            activeCastle.facadeAlpha = 0;
...
function updateCastles(dt) {
  ...
  // Y position and bounce (Suspension)
...
function update(ts) {
  ...
  projectiles = projectiles.filter(p => {
    ...
    if(Math.random()<0.3) {
        burst(p.x, p.y, "rgba(200,200,200,0.5)", 1, false, 0); // extra smoke
    }
    ...
    if (p.x > tgt.x && p.x < tgt.x+tgt.w && p.y > tgt.y && p.y < tgt.y+tgt.h) {
      ...
      tgt.isDestroyed = true; // impact breaks facade
      
      let livingUnits = CASTLE_UNITS[p.owner==="blue"?"red":"blue"].filter(u=>u.hp>0);
      if(livingUnits.length > 0) {
        // Find closest unit
        livingUnits.sort((a,b) => {
            let dax = Math.abs(p.x - (tgt.x + tgt.w*a.relX));
            let dbx = Math.abs(p.x - (tgt.x + tgt.w*b.relX));
            return dax - dbx;
        });
        let u = livingUnits[0];
        u.hp -= p.dmg;
  ...
  particles = particles.filter(p => {
    ...
    if(p.grav) p.vy+=400*dt; // debris gravity
    p.life-=dt*1.2; 
    return p.life>0;
  });
...
function drawCracks(c) {
  ...
  for(let i=0; i<c.cracks.length; i++) {
    ...
    for(let j=1; j<pts.length; j++) {
      ctx.lineWidth = 4 - (j/pts.length)*3; // taper width
...
function drawCastle(c, isRed) {
  ...
  if (c.isDestroyed) {
      // we can draw some fragments on the edges if needed
      drawCracks(c);
  }
...
function drawProjectiles() {
  projectiles.forEach(p => {
    ...
    if(p.trail.length>1) {
      ctx.beginPath();
      ctx.strokeStyle = "rgba(150,150,150,0.6)"; // Smoke trail
...
function explosion(x, y) {
  burst(x, y, "#ffffff", 10, false, 200); // Flash
  burst(x, y, "#f39c12", 20, false, 150); // Fire
  burst(x, y, "#555555", 15, true, 300);  // Debris
...
function drawParticles() {
  particles.forEach(p => {
    ...
    if (p.type === "shockwave") {
       ...
       ctx.fillRect(p.x, p.y, p.r*2, p.r*2); // square debris
    } else {
       ctx.fillStyle=p.color;
...
fs.writeFileSync(file4, head + newLogic);
